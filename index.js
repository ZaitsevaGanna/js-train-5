...
function findMinMax([firstElement, ...restElements]) {
  let resMin = [firstElement];
  let resMax = [firstElement];
  for (const i of restElements) {
    if (restElements[i] > resMax) {
      resMax = restElements[i];
    }
    if (restElements[i] < resMin) {
      resMin = restElements[i];
    }
  }

  return console.log(`Min: ${resMin}, Max: ${resMax}`);
...
function swapMinMax(array) {
  ...
  for (const i of array) {
    if (array[i] >= resMax) {
      resMax = array[i];
      maxIndex = i;
    } else if (array[i] < resMin) {
      resMin = array[i];
      minIndex = i;
    }
  }

  array[maxIndex] = resMin;
  array[minIndex] = resMax;
...
function multiplyByIndex(arr) {
  let resArray = [];
  for (const n in arr) {
    resArray[resArray.length] = n * arr[n];
  }
  console.log(resArray);
...
function replaceNumbers(arr) {
  ...
  for (const n of arr) {
    if (n > 10) {
      resArray[resArray.length] = "Greater than 10";
    } else if (n <= 10) {
      resArray[resArray.length] = "Less than or equal to 10";
    }
  }
  console.log(resArray);
}
